...
public class TestVehicles {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < otherVehicles.length; i++) {
			...
			if(otherVehicles[i] instanceof Vehicle){
			Vehicle v10 = (Vehicle) otherVehicles[i];
			v10.move((int) (Math.random() * 100));
			}//if
			if(otherVehicles[i] instanceof Ship){
				Ship s2 = (Ship) otherVehicles[i];
				s2.move((int) (Math.random() * 100));
			}//if
		}//for
